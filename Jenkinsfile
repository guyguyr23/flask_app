        ...
        stage('Remote SSH') {
            steps{
                sh '''
                 ...
                 if [ "$( docker container inspect -f '{{.State.Running}}' container_name2 )" == "true" ]; then docker rm container_name2 -f; fi
                 docker run -itd -p 5000:5000 --name container_name2 848215208608.dkr.ecr.us-east-1.amazonaws.com/flask_app"    
                 '''
            }
        }
    }
